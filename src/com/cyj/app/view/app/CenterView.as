...
//				info.path, info.isDirRes, info.dir, info.act
				...
				avt.x = info.x;
				avt.y = info.y;
				if(info.type == EffectPlayOwnerType.Sender || info.type == EffectPlayOwnerType.MyTeam || info.type == EffectPlayOwnerType.ShengWu)
				{
					roles.unshift(avt);
				}else{
					roles.push(avt);
		...
		private function refushMoveList():void
		{
			...
			if(data && data.move.type != EffectPlayOwnerType.None)
			{
				...
				if(roleLayer.owner && roles)
				{
					...
					if(data.effOwnerType == EffectPlayOwnerType.Sender || data.effOwnerType ==  EffectPlayOwnerType.MyTeam || data.effOwnerType ==  EffectPlayOwnerType.ShengWu){
						sends = owners;
						targets = roles;
					}else{
						sends = roles;
